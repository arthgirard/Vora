...
compenser la baisse naturelle de ce réflexe lors d'une concentration intense devant un écran. Pour assurer la robustesse de cette détection, l'algorithme intègre un mécanisme d'adaptation dynamique : à chaque clignement confirmé, le seuil de détection est recalibré en fonction de l'amplitude des pics passés du signal \textit{EARM} (signal d'ouverture des yeux filtré), permettant au système de s'ajuster en continu aux conditions lumineuses et à la morphologie propre de chaque utilisateur. D'ordre plus global, le fonctionnement est simple : le programme calcule une moyenne de la fréquence palpébrale et déclenche des notifications discrètes si l'hydratation oculaire est jugée insuffisante.
...
Dans la conception de Vora, Francois a principalement travaillé sur l’extraction des données à l’aide de la librairie MediaPipe. Cette extraction avait comme rôle d’extraire une liste de dictionnaires retournant les coordonnées des points sur le visage. De plus, François s'est occupé de créer le système de calcul de fréquence palpébrale en créant le gestionnaire de minuteurs pour ce volet. Par ailleurs, celui-ci a également conçu le système derrière la gestion des données, en créant la base de données SQLite3 et la gestion de celle-ci.  

\subsubsection{Maxime Vanseveren}

Maxime a principalement été responsable de la confection de l’interface graphique avec la bibliothèque Flet. Avec l’aide d’Arthur, Maxime a aussi fusionné l’algorithme d’arrière-plan (backend) à l’interface utilisateur (frontend) à l’aide de la librairie base64, permettant l’affichage en temps réel des graphiques et de la caméra dans les onglets « Analyse » et « Démarrage » respectivement. 

\vfill 

\begin{leftbar}
    \textbf{NOTE :} Des outils d'intelligence artificielle générative intégrés, tels que Gemini et Claude Code, ont été sollicités tout au long du développement de ce projet. Ces interactions ayant eu lieu au sein de multiples discussions dispersées, il est matériellement impossible d'en répertorier et d'en partager chaque lien individuel. Toutefois, pour garantir une transparence totale, l'intégralité du code généré ou assisté par une IA a été systématiquement identifiée par un marquage direct au sein des fichiers sources.
\end{leftbar}
...
Le problème de la détection des points de repère faciaux (Landmarks) en vision par ordinateur a été abordé de multiples manières au cours des trente dernières années. Avant l'émergence des réseaux de neurones, une approche courante pour identifier des objets ou le corps humain reposait sur les structures picturales \cite{felzenszwalb2005pictorial}, qui modélisaient les formes par un assemblage de parties déformables (souvent représentées par des rectangles). Cette méthode s'est toutefois révélée inefficace pour la détection faciale, car elle manquait de la précision nécessaire pour localiser des points dans des zones exigeant une grande précision, comme le contour des yeux ou des paupières.

L'essor des réseaux de neurones, et particulièrement des réseaux de neurones convolutifs (CNN), a permis d'augmenter significativement la précision de ces détections. Notamment, le modèle Face Mesh de Google permet d’identifier plus de 468 points sur le visage. Bien que des nouvelles versions portent des améliorations (478 points) \cite{ablavatski2020attention}, ce rapport se concentrera sur le modèle de base, détaillé dans l'étude Real-time Facial Surface Geometry from Monocular Video on Mobile GPUs \cite{kartynnik2019realtime}.\newline
Le modèle peut être séparé en plusieurs étapes :

\begin{enumerate}
  \item Le flux vidéo est d’abord traité par BlazeFace \cite{bazarevsky2019blazeface}, un détecteur de visage ultra-léger basé sur une architecture SSD (Single Shot Multi Box Detector). Son rôle est d'identifier la boîte englobante (bouding box) du visage pour isoler la région d'intérêt (ROI)
    \item Recadrage: pour que le système ne soit pas perturbé par les mouvements de l'utilisateur, il se corrige. Chaque fois que le visage est éloigné de la caméra, cette étape permet de redimensionner le visage détecté. Cette étape revient donc à changer le référentiel pour s'adapter à la position du visage.
...
$$\frac{\partial a_i}{\partial a_{i-1}} = \frac{\partial a_i}{\partial z_{i}} \cdot \frac{\partial z_{i}}{\partial a_{i-1}} = (\sigma^\prime(z_i) \cdot w_i)$$

Pour un réseau de $L$ couches, la dérivée partielle de la fonction de coût $C$ (ex: MSE) par rapport à un poids spécifique $w_1$ est obtenue par la règle de dérivation en chaîne. Le \[ \frac{\partial C}{\partial w_1}\] représente une composante du gradient $w_1$ qui fait partie du $\nabla_{\mathbf{w}} C$ utilisé pour la descente de gradient pour minimiser un $w$ quelconque. Donc,
...
Le gradient « disparaît » (la composante pour $w_1$), il devient une valeur minime : 
\[ (0,5)^{49} \approx 10^{-15} \]
...
Ce schéma représente un bloc résiduel. C'est ce mécanisme qui permet d'entraîner des réseaux de neurones profonds sans perdre le signal du gradient. L'entrée $x$ traverse d'abord les couches de calcul. Ce qui est retourné à la fin de ce parcours est la fonction résiduelle $f(x)$.La flèche supérieure représente le saut de connexion. C'est la « skip connection ». L'entrée $x$ est copiée et « saute » les couches de calcul pour être ajoutée directement plus loin. La sortie finale est donc l'addition des deux chemins :
\[ h(x) = f(x) + x \]

On définit la sortie d'un bloc $i$ (dans un réseau composé de $K$ blocs successifs) par :$$H_i = F_i(H_{i-1}) + H_{i-1}$$Où $F_i$ représente ce que retourne les couches internes du bloc. En dérivant par rapport à l'entrée du bloc, on obtient la dérivée partielle:
$$\frac{\partial H_i}{\partial H_{i-1}} = \frac{\partial F_i}{\partial H_{i-1}} + 1$$

Pour un réseau à $K$ blocs, le gradient total par rapport au premier bloc est :
$$\frac{\partial C}{\partial H_1} = \frac{\partial C}{\partial H_K} \cdot \prod_{i=2}^{K} \left( \frac{\partial F_i}{\partial H_{i-1}} + 1 \right)$$
Donc, 
$$\frac{\partial C}{\partial H_1} = \frac{\partial C}{\partial H_K} \cdot \prod_{i=2}^{K} (F^\prime_i + 1)$$

Maintenant, si on veut avoir l'impact du premier poids comme dans le premier exemple:
$$\frac{\partial C}{\partial w_1} = \frac{\partial C}{\partial H_1} \cdot \frac{\partial H_1}{\partial w_1}$$

$$\frac{\partial C}{\partial w_1} = \frac{\partial C}{\partial H_K} \cdot \left( \prod_{i=2}^{K} (F^\prime_i + 1) \right) \cdot \frac{\partial F_1}{\partial w_1}$$

$$ \frac{\partial C}{\partial w_1} = \frac{\partial C}{\partial H_K} \cdot (F^\prime_K + 1) \cdot (F^\prime_{K-1} + 1) \cdot \dots \cdot (F^\prime_2 + 1) \cdot \frac{\partial F_1}{\partial w_1} $$
Si nous reprenons l'exemple donné pour le problème du gradient qui disparaît, on remarque donc que le terme $+1$ permet de garantir que le gradient ne sera jamais inférieur à 1 (en supposant $F^\prime \geq 0$).Le réseau a $L$ couches. On pose que chaque bloc a 2 couches et on remplace $F^\prime$ par les poids réels :

$$ \frac{\partial C}{\partial w_1} = \frac{\partial C}{\partial H_K} \cdot \prod_{i=2}^{K} \left( w_{2i} \cdot \sigma^\prime(z_{2i}) \cdot w_{2i-1} \cdot \sigma^\prime(z_{2i-1}) + 1 \right) \cdot \left( w_2 \cdot \sigma^\prime(z_2) \cdot \sigma^\prime(z_1) \cdot a_0 \right) $$
...
\begin{enumerate}
    \item \textbf{En haut à gauche :} $(5 \cdot 1) + (2 \cdot 0) + (0 \cdot 0) + (4 \cdot 1) = 9$
    \item \textbf{En haut à droite :} $(2 \cdot 1) + (1 \cdot 0) + (4 \cdot 0) + (2 \cdot 1) = 4$
    \item \textbf{En bas à gauche :} $(0 \cdot 1) + (4 \cdot 0) + (3 \cdot 0) + (1 \cdot 1) = 1$
    \item \textbf{En bas à droite :} $(4 \cdot 1) + (2 \cdot 0) + (1 \cdot 0) + (6 \cdot 1) = 10$
\end{enumerate}
?
Note : Cet exemple est grandement inspiré de l'article de Jorge Cardete sur Medium : « Convolutional Neural Networks: A Comprehensive Guide » \cite{cardete2024cnn}
...
Lorsqu'une couche convolutive va sortir ses données, elle peut non seulement être sortie dans un vecteur à une dimensions, mais également sous forme de tenseur pour passer à travers une fonction d'activation commme ReLU.
...
\begin{enumerate}
    \item Les tenseurs provenant d'une image sont rapidement réduits à l'aide de couches convolutives. Ce processus est agressif afin de réduire les données du tenseur en des données qui représentent une fonctionnalité. Par exemple,le contour autour des yeux.
    \item Les données sont ensuite passées dans des blocs résiduels qui permettent un réseau de neurone extrêmement profond sans avoir ce soucié des problèmes de gradient qui disparaît.
    \item Finalement, dans les dernières couches du réseau, un problème de régression est résolu afin de retourner les coordonnées des points sur le visage de l'utilisateur.
\end{enumerate}
...
model.summary()
\end{lstlisting}
\caption{Architecture du réseau de neurones résiduel pour la régression comme Facemesh. Code généré par Gemini (\href{https://gemini.google.com/share/fc14f3a6bf1c}{https://gemini.google.com/share/fc14f3a6bf1c}).}
...
% BIBLIOGRAPHIE
\newpage
% SOURCES UTILISÉES MAIS JAMAIS VRAIMENT MENTIONNÉES
\nocite{girard2026vora}
\nocite{bulat2017alignment}
\nocite{ibm2021cnn}
\nocite{bytequest2025cnn}
\nocite{bryce2022resnet}
\nocite{3b1b2017backprop}
\nocite{bytequest2025gradient}
\nocite{devhutt2021vanishing}
\nocite{sandhu2025resnet}
\printbibliography
...
Résultats et\newline discussion & François & Interprétation des résultats, comparaisons à la littérature, analyse des limites du projet.  \\
